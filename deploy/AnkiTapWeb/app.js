...
const elements = {
  ...
  formulaListViewButton: document.querySelector("#formulaListViewButton"),
  formulaCategorySelect: document.querySelector("#formulaCategorySelect"),
  ...
  cardListView: document.querySelector("#cardListView"),
  cardListSearch: document.querySelector("#cardListSearch"),
  cardListFavoriteOnlyToggle: document.querySelector("#cardListFavoriteOnlyToggle"),
  cardListCondition: document.querySelector("#cardListCondition"),
  cardListCount: document.querySelector("#cardListCount"),
  cardListEmpty: document.querySelector("#cardListEmpty"),
...
function bindEvents() {
  ...
  elements.formulaCardViewButton.addEventListener("click", showFormulaCardView);
  elements.formulaListViewButton.addEventListener("click", showFormulaListView);
...
async function loadCards() {
  try {
    ...
    state.allCards = parseDelimitedCards(text).map((card, index) => ({
      ...card,
      listNumber: index + 1
    }));
...
function hideFormulaListInterface() {
  elements.formulaViewTools.classList.add("hidden");
  elements.cardListView.classList.add("hidden");
}
...
function showModeSelection() {
  ...
  state.formulaView = "card";
  state.cardListSearchText = "";
  state.cardListScrollTop = 0;
...
function formulaCategories() {
  const namedCategories = [...new Set(state.allCards.map(card => card.category).filter(Boolean))]
    .sort((a, b) => a.localeCompare(b, "ja"));
  return [ALL_CATEGORIES, ...namedCategories];
}

function formulaCategoryLabel(category) {
  return category === ALL_CATEGORIES ? "すべて" : (category || "カテゴリを選択");
}

function populateFormulaCategorySelect() {
  const placeholder = document.createElement("option");
  placeholder.value = "";
  placeholder.textContent = "カテゴリを選択";

  const options = formulaCategories().map(category => {
    const option = document.createElement("option");
    option.value = category;
    option.textContent = formulaCategoryLabel(category);
    return option;
  });

  elements.formulaCategorySelect.replaceChildren(placeholder, ...options);
  elements.formulaCategorySelect.value = state.selectedCategory || "";
}

function updateFormulaViewControls() {
  const showingList = state.formulaView === "list";
  elements.formulaCardViewButton.setAttribute("aria-pressed", String(!showingList));
  elements.formulaListViewButton.setAttribute("aria-pressed", String(showingList));
  elements.formulaCategorySelect.value = state.selectedCategory || "";
}
...
function renderCategories() {
  ...
  updateFavoriteOnlyControls();
  populateFormulaCategorySelect();
  updateFormulaViewControls();
  ...
  elements.categoryList.replaceChildren(...categories.map(category => {
    ...
    button.type = "button";
    button.innerHTML = `<strong></strong><span></span><span aria-hidden="true">›</span>`;
    button.querySelector("strong").textContent = formulaCategoryLabel(category);
    button.querySelector("span").textContent = `${countEligibleCards(category)}枚`;
    button.addEventListener("click", () => selectCategory(category));
    return button;
  }));

  elements.formulaViewTools.classList.remove("hidden");
  elements.cardListView.classList.add("hidden");
  elements.emptyView.classList.add("hidden");
  elements.modeView.classList.add("hidden");
  elements.studyView.classList.add("hidden");
...
function selectCategory(category, cardKey = null) {
  state.formulaView = "card";
  state.selectedCategory = category;
  state.cards = getEligibleCards(category);
  const requestedIndex = cardKey
    ? state.cards.findIndex(card => favoriteCardKey(card) === cardKey)
    : -1;
...
function showFormulaCardView() {
  ...
  selectCategory(state.selectedCategory, currentCard ? favoriteCardKey(currentCard) : null);
}

function showFormulaListView() {
  if (state.mode !== "formula") {
    return;
  }

  state.formulaView = "list";
  state.selectedCategory ||= ALL_CATEGORIES;
  populateFormulaCategorySelect();
  updateFormulaViewControls();
  updateFavoriteOnlyControls();
  elements.cardListSearch.value = state.cardListSearchText;
  ...
  requestAnimationFrame(() => window.scrollTo({ top: state.cardListScrollTop, behavior: "auto" }));
}

function handleFormulaCategoryChange(event) {
...
function handleCardListFavoriteOnlyChange(event) {
...
function handleCardListSearchInput(event) {
  state.cardListSearchText = event.target.value;
  state.cardListScrollTop = 0;
  clearTimeout(cardListSearchTimer);
...
function normalizedCardListText(value) {
  return String(value ?? "").normalize("NFKC").toLocaleLowerCase("ja");
}

function cardListTextContent(value) {
...
function cardListExcerpt(value) {
  return cardListTextContent(value).replace(/\s+/g, " ").trim();
}

function getFilteredCardList() {
  const category = state.selectedCategory || ALL_CATEGORIES;
  const searchText = normalizedCardListText(state.cardListSearchText.trim());

  return state.allCards.filter(card => {
    if (!matchesCategory(card, category) || (state.favoriteOnly && !isFavoriteCard(card))) {
      return false;
    }
    ...
    const searchableText = normalizedCardListText([
      cardListTextContent(card.question),
      cardListTextContent(card.answer),
      cardListTextContent(card.explanation),
      card.category || ""
    ].join("\n"));
...
function renderCardList() {
  const cards = getFilteredCardList();
  const condition = [formulaCategoryLabel(state.selectedCategory || ALL_CATEGORIES)];

  if (state.favoriteOnly) {
    condition.push("お気に入り");
  }
  if (state.cardListSearchText.trim()) {
    condition.push("検索結果");
  }

  elements.cardListCondition.textContent = condition.join(" / ");
  elements.cardListCount.textContent = `${cards.length}件`;
  elements.cardListEmpty.classList.toggle("hidden", cards.length > 0);
  elements.cardList.classList.toggle("hidden", cards.length === 0);

  if (cards.length === 0) {
    elements.cardListEmpty.textContent = state.cardListSearchText.trim()
      ? "検索条件に一致する問題はありません。"
      : state.favoriteOnly
        ? "このカテゴリにはお気に入り登録された問題がありません。"
        : "該当する問題はありません。";
  ...
  const rows = cards.map(card => {
    ...
    favoriteButton.className = "card-list-favorite";
    favoriteButton.type = "button";
    favoriteButton.textContent = favorite ? "★" : "☆";
    favoriteButton.setAttribute("aria-pressed", String(favorite));
    favoriteButton.setAttribute(
      "aria-label",
      favorite ? `No.${card.listNumber}をお気に入りから解除` : `No.${card.listNumber}をお気に入りに登録`
    );
    ...
    openButton.className = "card-list-open";
    openButton.type = "button";
    openButton.setAttribute("aria-label", `No.${card.listNumber} ${cardListExcerpt(card.question)}をカードで開く`);

    const number = document.createElement("span");
    number.className = "card-list-number";
    number.textContent = `No.${card.listNumber}`;

    const question = document.createElement("strong");
    question.className = "card-list-question";
    question.textContent = cardListExcerpt(card.question);

    const answer = document.createElement("span");
    answer.className = "card-list-answer";
    answer.textContent = cardListExcerpt(card.answer);

    openButton.append(number, question, answer);
    openButton.addEventListener("click", () => openCardFromList(card));
    item.append(favoriteButton, openButton);
    return item;
  });

  elements.cardList.replaceChildren(...rows);
}

function openCardFromList(card) {
  state.cardListScrollTop = window.scrollY;
  selectCategory(state.selectedCategory || ALL_CATEGORIES, favoriteCardKey(card));
}
...
function updateFavoriteOnlyControls() {
  elements.favoriteOnlyToggle.checked = state.favoriteOnly;
  elements.cardListFavoriteOnlyToggle.checked = state.favoriteOnly;
  const category = state.selectedCategory || ALL_CATEGORIES;
...
function toggleFavoriteCard(card) {
  ...
  updateFavoriteOnlyControls();
  return state.favoriteCardKeys.has(key);
}
